...
| **Основные шаги сценария** | 1) Клиент выбирает подходящий столик из списка доступных;<br>2) Система отображает форму для ввода контактных данных клиента (имя, телефон);<br>3) Клиент вводит контактные данные и подтверждает создание бронирования;<br>4) Система направляет запрос в API-сервис на создание бронирования;<br>5) 5) API-сервис обращается к базе данных и формирует список столиков, которые имеют достаточную вместимость и свободны в выбранный двухчасовой период;
;<br>6) API-сервис сохраняет данные о бронировании в базе данных и присваивает бронированию уникальный идентификатор;<br>7) Система отображает клиенту подтверждение выполненной операции с указанием даты, времени, выбранного столика и идентификатора бронирования |
...
При создании или изменении бронирования система должна проверять его на пересечение с другими действующими бронированиями выбранного столика.
При изменении бронирования само редактируемое бронирование в проверке пересечений не учитывается.
...
### 4.1.7 Управление бронированиями администратором
Администратор должен иметь возможность:
1) просматривать список бронирований;
2) просматривать информацию о конкретном бронировании;
3) создавать бронирования;
4) изменять бронирования;
5) отменять бронирования.
При создании или изменении бронирования администратором должны применяться те же правила проверки доступности столика, что и при бронировании клиентом.
### 4.1.8 Управление столиками
Администратор должен иметь возможность:
1) просматривать список столиков;
2) добавлять новые столики;
3) изменять данные столиков;
4) удалять столики.
Для каждого столика должны храниться:
1) уникальный идентификатор;
2) номер столика;
3) вместимость;
4) расположение или зона ресторана.
Столик не может быть удален, если с ним связаны будущие действующие бронирования.
Перед удалением такого столика связанные бронирования должны быть изменены или отменены.
...
Отмена бронирования не должна приводить к физическому удалению записи о нем из базы данных. Статус бронирования должен изменяться на "отменено".
...
7) изменения бронирования;
8) запрета удаления столика с действующими бронированиями;
9) обработки некорректных входных данных.
...
9) используется одна административная роль;
10) мобильное приложение не разрабатывается.
